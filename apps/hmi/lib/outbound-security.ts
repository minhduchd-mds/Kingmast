...
function privateIpLiteral(host: string) {
  ...
  // HMI code intentionally avoids node:net/raw-socket-capable imports. IPv6 literals
  // are conservatively denied by this policy; approved production providers use DNS names.
  if (value.includes(':')) return true;
  if (!ipv4Literal(value)) return false;
