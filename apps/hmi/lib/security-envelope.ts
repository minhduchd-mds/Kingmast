...
export type NavigationScope = 'navigation:route' | 'navigation:search';
...
type SecurityStore = { buckets: Map<string, Bucket>; capabilityReplay: Map<string, ReplayEntry>; paidDay: string; paidCalls: number };

const MAX_RATE_KEYS = 4096;
const globalSecurity = globalThis as typeof globalThis & { __kingmastSecurityEnvelope?: SecurityStore };
const store: SecurityStore = globalSecurity.__kingmastSecurityEnvelope ?? { buckets: new Map(), capabilityReplay: new Map(), paidDay: '', paidCalls: 0 };
...
function clientIdentity(request: NextRequest) {
  ...
  const forwarded = (request.headers.get('x-vercel-forwarded-for') ?? request.headers.get('x-forwarded-for') ?? request.headers.get('x-real-ip') ?? 'unknown').split(',')[0]?.trim();
...
function capabilityCanonical(scope: NavigationScope, method: string, pathname: string, expiresAt: number, nonce: string) {
  return `${scope}\n${method.toUpperCase()}\n${pathname}\n${expiresAt}\n${nonce}`;
}

export function issueNavigationCapability(scope: NavigationScope, method: 'GET' | 'POST', pathname: string, nowMs = Date.now()) {
  const secret = navSecret();
  if (!secret) return null;
  if (!pathname.startsWith('/api/kingmast/live-navigation/')) return null;
  const ttlSeconds = boundedInt('KINGMAST_NAV_CAPABILITY_TTL_SECONDS', 120, 15, 300);
  const expiresAt = nowMs + ttlSeconds * 1000;
  const nonce = randomBytes(18).toString('base64url');
  const signature = createHmac('sha256', secret).update(capabilityCanonical(scope, method, pathname, expiresAt, nonce)).digest('hex');
  return { token: `v1.${expiresAt}.${nonce}.${signature}`, expiresAt };
}

function pruneRateBuckets(now: number) {
  if (store.buckets.size < MAX_RATE_KEYS / 2) return;
  for (const [key, value] of store.buckets) if (value.resetAt <= now) store.buckets.delete(key);
}
...
function pruneReplay(now: number) {
  ...
  for (const [key, value] of store.capabilityReplay) if (value.expiresAt <= now) store.capabilityReplay.delete(key);
...
function verifyCapability(request: NextRequest, scope: NavigationScope) {
  const secret = navSecret();
  ...
  const expected = createHmac('sha256', secret).update(capabilityCanonical(scope, request.method, request.nextUrl.pathname, expiresAt, nonce)).digest('hex');
...
export function admitNavigationRequest(request: NextRequest, scope: NavigationScope): Admission {
...
export function consumePaidRoutingQuota() {
  ...
  if (store.paidDay !== day) { store.paidDay = day; store.paidCalls = 0; }
...
export function securityEnvelopeHeaders(reason?: string) {
  return { 'cache-control': 'no-store', 'x-kingmast-security-envelope': 'enforced', ...(reason ? { 'x-kingmast-security-reason': reason } : {}) };
}
