...
expect('cloud TTS requires explicit opt-in',tts.includes("KINGMAST_CLOUD_TTS_ENABLED")&&tts.includes("error:'cloud-tts-disabled'"));
expect('TTS provider egress is centrally allowlisted',tts.includes("approvedOutboundUrl(raw,'tts-self-host')")&&tts.includes("'tts-openai'")&&tts.includes("'tts-elevenlabs'")&&outbound.includes("'api.openai.com'")&&outbound.includes("'api.elevenlabs.io'"));
...
expect('cloud AI requires explicit opt-in',provider.includes("KINGMAST_CLOUD_AI_ENABLED")&&provider.includes("cloudAiEnabled:enabled('KINGMAST_CLOUD_AI_ENABLED')"));
expect('external AI provider uses central egress allowlist',provider.includes("approvedOutboundUrl(raw,'assistant-provider')")&&outbound.includes("'assistant-provider'"));
expect('production risk-engine access is local by default',provider.includes("KINGMAST_PUBLIC_RISK_ENGINE_ENABLED")&&provider.includes("loopback(url.hostname)"));
