...
function dispatchRuntime(page:Page,dmsState:'attentive'|'distracted'='attentive',cameraState:'ok'|'degraded'|'unavailable'='ok'){
  return page.evaluate(({state,sensorCamera})=>{
    const now=Date.now();
    const assist={
      generatedAtMs:now,
      ...
      surround:{availability:'live' as const,observedAtMs:now,ageMs:30,cameraCount:4,calibratedCameraCount:4,synchronizedCameraCount:4,readyCameraCount:4,maxReprojectionErrorPx:1.7,calibrationUncertaintyPx:1.7,geometryConfidence:.82,fullyReady:true,reason:'surround-calibration-ready',visualizationOnly:true as const},
    };
    const frame={
      sequence:42,
      vehicle:{lat:21.0285,lng:105.8542,speedKmh:48,headingDeg:0,accuracyM:2,timestampMs:now,source:'gnss' as const},
      sensors:{radarFront:'ok' as const,radarRear:'ok' as const,camera:sensorCamera,can:'ok' as const,gnssImu:'ok' as const,ecu:'ok' as const},
      objects:[],
      alerts:[],
      assist,
    };
    window.dispatchEvent(new CustomEvent('kingmast:telemetry',{detail:{frame,receivedAtMs:now,diagnostics:null}}));
  },{state:dmsState,sensorCamera:cameraState});
}
...
test.describe('driver-facing assistance capability rail',()=>{
  ...
  test('collapses normal secondary status while moving and keeps attention-relevant assist state',async({page})=>{
    ...
    await expect(rail.locator('[data-capability="vehicle"]')).toHaveCount(0);
  ...
  test('surfaces degraded sensor health while driving without adding another persistent widget',async({page})=>{
    await openReturningDriver(page,1366,768);
    await dispatchRuntime(page,'attentive','degraded');
    const rail=page.getByTestId('driver-capability-rail');
    const vehicle=rail.locator('[data-capability="vehicle"]');
    await expect(vehicle).toBeVisible();
    await expect(vehicle).toContainText('Sensor health degraded');
    await expect(vehicle).toContainText('Camera degraded');
    await expect(vehicle).toHaveAttribute('data-runtime-state','degraded');
    await expect(rail.locator('.driverCapabilityItem')).toHaveCount(2);
  });
