...
test.describe('Surround visualization v2',()=>{
  test('1366x768 prioritizes spatial object awareness without duplicate directional badges',async({page})=>{
  ...
  test('1920x720 uses width for the surround canvas instead of extra driving cards',async({page})=>{
    ...
    const visibleMarkers=page.getByTestId('surround-spatial-layer').locator('.surroundMarker');
    expect(await visibleMarkers.count()).toBeGreaterThanOrEqual(2);
    expect(await hasHorizontalOverflow(page)).toBe(false);
  });

  test('1280x480 keeps the surround scene readable and sheds secondary metrics',async({page})=>{
