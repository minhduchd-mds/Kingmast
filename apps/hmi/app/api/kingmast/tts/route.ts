...
function safeSelfHostUrl(){const raw=(process.env.KINGMAST_TTS_SELF_HOST_URL??'').trim();if(!raw)return null;try{const url=new URL(raw);if(url.protocol==='https:'||(url.protocol==='http:'&&isLoopback(url.hostname)))return url;return null;}catch{return null;}}
...
async function openAiSpeech(input:{text:string;locale:string;voice:string;moving:boolean;style:string}){
  ...
  const response=await fetch('https://api.openai.com/v1/audio/speech',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${key}`},body:JSON.stringify(payload),cache:'no-store',signal:AbortSignal.timeout(6500)});
...
async function elevenLabsSpeech(input:{text:string;voice:string}){
  ...
  const response=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(voice)}?output_format=mp3_44100_64`,{method:'POST',headers:{'content-type':'application/json','xi-api-key':key},body:JSON.stringify({text:input.text,model_id:model,voice_settings:{stability:.34,similarity_boost:.78,style:.3,use_speaker_boost:true}}),cache:'no-store',signal:AbortSignal.timeout(6500)});
...
export async function POST(request:NextRequest){
  ...
  const requested=requestedProvider(body.voice);const providers=requested?[requested,...configuredProviders().filter((item)=>item!==requested)]:configuredProviders();
