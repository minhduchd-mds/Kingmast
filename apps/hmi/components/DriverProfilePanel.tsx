...
export default function DriverProfilePanel(){
  ...
  return <section className="systemCard" data-testid="driver-profile-controls">
    ...
    <div className="profileGrid">
      ...
      <div className="profileSegment"><span><Ruler/> Unit system</span><div role="group" aria-label="Unit system"><button type="button" aria-pressed={profile.units==='metric'} className={profile.units==='metric'?'selected':''} onClick={()=>updateProfile({units:'metric'})}>Metric</button><button type="button" aria-pressed={profile.units==='imperial'} className={profile.units==='imperial'?'selected':''} onClick={()=>updateProfile({units:'imperial'})}>Imperial</button></div><small>Driver-facing safety speed and distance readouts switch together across Drive, Navigation, camera, alert and connected-road surfaces. Internal risk calculations remain SI.</small></div>
    </div>
