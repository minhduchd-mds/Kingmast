...
export default function NativeNavigationMap({vehicle,objects,cameras,route,headingUp,compact}:Props){
  ...
  useEffect(()=>{const map=mapRef.current;if(!map||rendererFailure)return;try{dynamicMarkers.current.forEach((marker)=>marker.remove());const next:Marker[]=[];for(const object of objects.slice(0,24)){const marker=new Marker({element:markerElement(object.kind,objectTone(object),`${object.kind}, ${Math.round(object.distanceM)} meters`),anchor:'center'}).setLngLat([object.position.lng,object.position.lat]).addTo(map);next.push(marker);}for(const camera of cameras.slice(0,40)){const marker=new Marker({element:markerElement('camera',camera.kind==='speed-enforcement'||camera.kind==='average-speed'?'caution':'neutral',camera.kind),anchor:'center'}).setLngLat([camera.position.lng,camera.position.lat]).addTo(map);next.push(marker);}if(route){const point=route.destination;next.push(new Marker({element:markerElement('destination','primary','Destination'),anchor:'bottom'}).setLngLat([point.lng,point.lat]).addTo(map));}dynamicMarkers.current=next;}catch{setRendererFailure('map-marker-update-failed');}},[cameras,objects,rendererFailure,route]);
