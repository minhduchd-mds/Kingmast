...
export default function NativeNavigationMap({vehicle,objects,cameras,route,headingUp,compact}:Props){
  const containerRef=useRef<HTMLDivElement|null>(null);const mapRef=useRef<MapLibreMap|null>(null);const vehicleMarker=useRef<Marker|null>(null);const dynamicMarkers=useRef<Marker[]>([]);const[rendererFailure,setRendererFailure]=useState<string|null>(null);const{isVietnamese}=useI18n();const tx=(en:string,vi:string)=>isVietnamese?vi:en;
  ...
  useEffect(()=>{
    ...
    try{
      map=new MapLibreMap({container:containerRef.current,style:mapStyle(),center:[vehicle.lng,vehicle.lat],zoom:compact?15:16,bearing:headingUp?vehicle.headingDeg:0,pitch:compact?0:22,attributionControl:{compact:true},interactive:!compact});
  ...
  useEffect(()=>{const map=mapRef.current;if(!map||rendererFailure)return;try{dynamicMarkers.current.forEach((marker)=>marker.remove());const next:Marker[]=[];for(const object of objects.slice(0,24)){const marker=new Marker({element:markerElement(object.kind,objectTone(object),isVietnamese?`${object.kind}, cách ${Math.round(object.distanceM)} mét`:`${object.kind}, ${Math.round(object.distanceM)} meters`),anchor:'center'}).setLngLat([object.position.lng,object.position.lat]).addTo(map);next.push(marker);}for(const camera of cameras.slice(0,40)){const marker=new Marker({element:markerElement('camera',camera.kind==='speed-enforcement'||camera.kind==='average-speed'?'caution':'neutral',camera.kind),anchor:'center'}).setLngLat([camera.position.lng,camera.position.lat]).addTo(map);next.push(marker);}if(route){const point=route.destination;next.push(new Marker({element:markerElement('destination','primary',tx('Destination','Điểm đến')),anchor:'bottom'}).setLngLat([point.lng,point.lat]).addTo(map));}dynamicMarkers.current=next;}catch{setRendererFailure('map-marker-update-failed');}},[cameras,isVietnamese,objects,rendererFailure,route]);

  if(rendererFailure)return <div className={`kingmastNativeMap ${compact?'compact':''}`} data-testid="map-renderer-fallback" data-map-failure={rendererFailure} aria-label={tx('Navigation map unavailable','Bản đồ dẫn đường không khả dụng')} style={{display:'grid',placeItems:'center',padding:20,textAlign:'center'}}><span><strong style={{display:'block',fontSize:14}}>{tx('Map renderer unavailable','Không thể hiển thị bản đồ')}</strong><small style={{display:'block',marginTop:4,opacity:.72}}>{tx('Navigation guidance and warning-only safety functions continue without the WebGL map surface.','Hướng dẫn dẫn đường và các chức năng an toàn chỉ cảnh báo vẫn tiếp tục hoạt động khi bề mặt bản đồ WebGL không khả dụng.')}</small></span></div>;
  return <div ref={containerRef} className={`kingmastNativeMap ${compact?'compact':''}`} aria-label={tx('KINGMAST navigation map','Bản đồ dẫn đường KINGMAST')}/>;
}
