...
import { Bot,CheckCircle2,LoaderCircle,MessageCircle,Mic,MicOff,Send,ShieldCheck,Speaker,SpeakerX,TriangleAlert,X } from 'lucide-react';
...
export default function AssistantOverlay(){
  ...
  return <div className={`assistantLayer ${open?'isOpen':''} ${moving?'isMoving':'isParked'}`} data-testid="kingmast-assistant">
    ...
    {open?<section className="assistantPanel" role="dialog" aria-modal="false" aria-label={t('assistant.title')}>
      <header className="assistantHeader"><span className="assistantOrb"><Bot/></span><span><strong>{t('assistant.title')}</strong><small>{t('assistant.tagline')}</small></span><div><button type="button" aria-label={speakReplies?t('assistant.speakOff'):t('assistant.speakOn')} aria-pressed={speakReplies} onClick={()=>{setSpeakReplies((value)=>!value);if(speakReplies&&'speechSynthesis'in window)window.speechSynthesis.cancel();}}>{speakReplies?<Speaker/>:<SpeakerX/>}</button><button type="button" aria-label={t('assistant.close')} onClick={()=>setOpen(false)}><X/></button></div></header>
