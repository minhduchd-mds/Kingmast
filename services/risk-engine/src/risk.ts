...
export function assessRisk(sample: VehicleSample, nowMs=Date.now()): RiskAssessment {
  ...
  if(!sample.canHealthy){ reasons.push('can-degraded'); if(confidence<0.9) return finish({severity:'caution',ttcS,thwS,closingSpeedMps,confidence:confidence*0.6,reasons}); }
