...
app.post('/v3/geofences',async(request,reply)=>{const authority=requireConfigurationAuthority(request,reply,'configuration:geofences',request.body);if(!authority)return;const parsed=GeofenceReplaceSchema.safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:'invalid-geofences',details:parsed.error.flatten()});const previous=geofences;const next=parsed.data.geofences as Geofence[];geofences=next;const audit=configurationAudit.record({actorId:authority.actorId,keyId:authority.keyId,authMode:authority.authMode,previousValue:previous,newValue:next,previousCount:previous.length,newCount:next.length,sourceIp:request.ip});return{updated:geofences.length,geofences,auditSequence:audit.sequence,authorization:authority.authMode};});
