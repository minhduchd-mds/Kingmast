...
await app.register(cors,{origin:process.env.HMI_ORIGIN??'http://localhost:3000',methods:['GET','POST'],credentials:true,allowedHeaders:['content-type','authorization','x-kingmast-edge-token','x-kingmast-viewer-token','x-kingmast-device-key-id','x-kingmast-device-signature']});
...
app.get('/health',async()=>({status:'ok',mode:'warning-only',version:'3.5-device-identity'}));
...
app.get('/v3/health/details',async(request,reply)=>{if(!requireViewerAuth(request,reply))return;return{status:'ok',mode:'warning-only',edge:diagnostics(),assist:driverAssistRuntime.snapshot(Date.now(),hasFreshVehicleContext()),deviceIdentity:{required:REQUIRE_DEVICE_AUTH,...deviceAuthSummary(DEVICE_KEYS)}};});
...
app.get('/v3/device-identity/status',async(request,reply)=>{if(!requireViewerAuth(request,reply))return;return{mode:REQUIRE_DEVICE_AUTH?'required':'transition',sharedEdgeTokenFallback:!REQUIRE_DEVICE_AUTH,algorithm:'HMAC-SHA256',packetBinding:['deviceId','keyId','bootId','sequence','timestampMs','canonical-packet'],...deviceAuthSummary(DEVICE_KEYS)};});
