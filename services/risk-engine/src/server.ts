...
await app.register(cors,{origin:process.env.HMI_ORIGIN??'http://localhost:3000',methods:['GET','POST'],credentials:true,allowedHeaders:['content-type','authorization','x-kingmast-edge-token','x-kingmast-viewer-token','x-kingmast-config-token','x-kingmast-device-id','x-kingmast-device-key-id','x-kingmast-device-signature']});
...
function requireConfigAuth(request:FastifyRequest,reply:FastifyReply){
  if(!configAuthorized(request)){reply.code(401).send({error:'configuration-auth-required'});return false;}
  const decision=configWriteLimiter.consume(request.ip,30);
  if(decision.allowed)return true;
  reply.header('retry-after',String(decision.retryAfterS)).code(429).send({error:'configuration-write-rate-limited',reason:decision.reason});
  return false;
}
...
app.get('/health',async()=>({status:'ok',mode:'warning-only',version:'3.9-config-auth'}));
...
app.get('/v3/health/details',async(request,reply)=>{if(!requireViewerAuth(request,reply))return;return{status:'ok',mode:'warning-only',edge:diagnostics(),assist:driverAssistRuntime.snapshot(Date.now(),hasFreshVehicleContext()),deviceIdentity:{required:REQUIRE_DEVICE_AUTH,...deviceAuthSummary(DEVICE_KEYS)},configuration:{writeEnabled:CONFIG_TOKEN.length>=32||ALLOW_INSECURE_LOCAL_DEV,authorization:CONFIG_TOKEN.length>=32?'dedicated-token':ALLOW_INSECURE_LOCAL_DEV?'loopback-dev':'disabled'},observability:{risk:riskMetricsSnapshot(),audit:eventBuffer.auditStatus(),runtimeGuards:runtimeGuardStatus()}};});
...
app.post('/v3/geofences',async(request,reply)=>{if(!requireConfigAuth(request,reply))return;const parsed=GeofenceReplaceSchema.safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:'invalid-geofences',details:parsed.error.flatten()});geofences=parsed.data.geofences as Geofence[];return{updated:geofences.length,geofences};});
