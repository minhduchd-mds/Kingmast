...
const VIEWER_COOKIE='kingmast_viewer';
...
function cookieToken(request:FastifyRequest){const raw=request.headers.cookie??'';for(const part of raw.split(';')){const [key,...rest]=part.trim().split('=');if(key===VIEWER_COOKIE)return decodeURIComponent(rest.join('='));}return '';}
...
function viewerAuthorized(request:FastifyRequest){if(localDevAuthorized(request))return true;if(!VIEWER_TOKEN)return false;const candidates=[headerToken(request,'x-kingmast-viewer-token'),bearerToken(request),cookieToken(request)].filter(Boolean);return candidates.some((candidate)=>constantTimeEqual(candidate,VIEWER_TOKEN));}
...
app.get('/health',async()=>({status:'ok',mode:'warning-only',version:'3.3-security-hardening'}));
app.post('/v3/session',async(request,reply)=>{if(!viewerAuthorized(request))return reply.code(401).send({error:'viewer-auth-required'});const secure=process.env.NODE_ENV==='production';reply.header('set-cookie',`${VIEWER_COOKIE}=${encodeURIComponent(VIEWER_TOKEN)}; Path=/; HttpOnly; ${secure?'Secure; SameSite=None':'SameSite=Lax'}; Max-Age=28800`);return{authenticated:true,expiresInS:28_800};});
app.get('/v3/health/details',async(request,reply)=>{if(!requireViewerAuth(request,reply))return;return{status:'ok',mode:'warning-only',edge:diagnostics()};});
...
app.get('/v1/capabilities',async()=>({vehicleControl:false,canWrite:false,brake:false,steer:false,throttle:false,gpsPositioning:true,objectDetection:true,radarFusion:true,cameraClassification:true,realtimeWebSocket:true,heartbeat:true,edgeReplayProtection:true,legacyReplayProtection:true,edgeAuthentication:EDGE_TOKEN.length>=16,viewerAuthentication:VIEWER_TOKEN.length>=16,diagnostics:true,eventHistory:true,geofenceAlerts:true,mapAlerts:true,navigationRouting:true,speedLimitAwareness:true,speedSignVision:true,trafficCameraContext:true,trafficCameraAccessPolicy:'public-or-authorized-only'}));
