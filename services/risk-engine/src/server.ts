...
app.get('/v1/capabilities',async()=>({vehicleControl:false,canWrite:false,brake:false,steer:false,throttle:false,gpsPositioning:true,objectDetection:true,radarFusion:true,cameraClassification:true,realtimeWebSocket:true,heartbeat:true,edgeReplayProtection:true,legacyReplayProtection:true,edgeAuthentication:EDGE_TOKEN.length>=16,viewerAuthentication:VIEWER_TOKEN.length>=16,diagnostics:true,eventHistory:true,geofenceAlerts:true,mapAlerts:true,navigationRouting:true,speedLimitAwareness:true,speedSignVision:true,trafficCameraContext:true,driverAssistRuntime:true,laneDepartureRuntime:true,driverMonitoringRuntime:true,surroundReadinessRuntime:true,readOnlyAssistantPlanner:true,trafficCameraAccessPolicy:'public-or-authorized-only'}));
