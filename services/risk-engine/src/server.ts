...
await app.register(cors,{origin:process.env.HMI_ORIGIN??'http://localhost:3000',methods:['GET','POST'],credentials:true,allowedHeaders:['content-type','authorization','x-kingmast-edge-token','x-kingmast-viewer-token','x-kingmast-device-id','x-kingmast-device-key-id','x-kingmast-device-signature']});
...
app.get('/health',async()=>({status:'ok',mode:'warning-only',version:'3.8-device-ingress'}));
