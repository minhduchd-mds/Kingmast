...
app.get('/v3/diagnostics',{config:{rateLimit:{max:120,timeWindow:60_000}}},async(request,reply)=>{if(!requireViewerAuth(request,reply))return;return{...diagnostics(),risk:riskMetricsSnapshot(),audit:eventBuffer.auditStatus(),configurationAudit:configurationAudit.status(),operatorIdentity:{required:REQUIRE_OPERATOR_AUTH,...operatorAuthSummary(OPERATOR_KEYS)},nextgen:nextgenRuntime.snapshot(),runtimeGuards:runtimeGuardStatus()};});
