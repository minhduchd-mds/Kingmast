...
import { calculateNavigationRoute } from './navigation.js';
...
export const roadContextRoutes:FastifyPluginAsync=async(app)=>{
  ...
  app.get('/v4/road-context/providers',async()=>({cameraProviders:configuredCameraProviderCount(),routingProvider:'osrm',publicMapProvider:'openstreetmap-overpass',policy:'Only public or explicitly authorized traffic-camera metadata/feed endpoints may be configured. KINGMAST does not bypass camera authentication.'}));
};
