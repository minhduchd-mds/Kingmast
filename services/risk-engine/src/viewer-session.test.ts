...
import { issueViewerSession,verifyViewerSession,VIEWER_SESSION_SCOPE,VIEWER_SESSION_TTL_S } from '@kingmast/contracts/viewer-session';
...
describe('scoped viewer session',()=>{
  ...
  it('rejects tampered and wrong-secret tokens',()=>{
    const token=issueViewerSession(secret,now);
    expect(verifyViewerSession(`${token.slice(0,-1)}x`,secret,now)).toBe(false);
  ...
  it('expires after the fixed session TTL and clock-skew allowance',()=>{
    ...
    expect(verifyViewerSession(token,secret,expiredAt)).toBe(false);
  });
