...
function matchCamera(track:RadarTrack,detections:CameraDetection[]) {
  let best:CameraDetection|null = null;
  let bestScore = Number.POSITIVE_INFINITY;
  for (const detection of detections) {
    const bearingScore = angleDelta(track.bearingDeg,detection.bearingDeg);
    if (bearingScore > MATCH_BEARING_DEG) continue;
    ...
    const score = bearingScore + distanceScore * 0.35;
    if (score < bestScore) { best = detection; bestScore = score; }
  }
  return best;
}
...
export function fuseEdgePerception(input:{
  ...
  for (const track of radarTracks) {
    const detection = matchCamera(track,cameraDetections);
    if (detection) usedCameraIds.add(detection.id);
    const kind:ObjectKind = detection?.kind ?? 'unknown';
    const confidence = Math.max(0,Math.min(1,detection ? Math.sqrt(track.confidence*detection.confidence) : track.confidence*0.72));
  ...
  for (const detection of cameraDetections) {
    ...
    objects.push({
      ...
      bearingDeg:worldBearing,
      zone:zoneForBearing(relativeBearing),
      severity:severityFor(detection.kind,distanceM,detection.confidence*0.75),
      relativeSpeedMps:0,
