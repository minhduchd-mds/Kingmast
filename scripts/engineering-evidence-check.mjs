...
if(!provenance.includes("schema:'kingmast-build-provenance/v1'")||!provenance.includes("actuatorAuthority:'none'")||!provenance.includes('KINGMAST_BUILD_PATHS')||!ci.includes('Generate build provenance')||!ci.includes('/tmp/kingmast.provenance.json')||!/actions\/upload-artifact@[a-f0-9]{40}/i.test(ci))failures.push('build provenance / pinned evidence upload contract missing');
