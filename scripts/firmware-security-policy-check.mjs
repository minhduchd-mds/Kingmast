...
if(!firmware.includes('KINGMAST-EDGE-V1')||!firmware.includes('mbedtls_md_hmac')||!firmware.includes('x-kingmast-device-key-id')||!firmware.includes('x-kingmast-device-signature'))failures.push('ESP32 consolidated edge publisher must preserve per-device HMAC packet signing');
if(!firmware.includes('Insert every object key in lexical order'))failures.push('ESP32 packet construction must preserve the research canonical serialization contract');
