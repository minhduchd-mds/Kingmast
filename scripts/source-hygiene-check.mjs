...
function walk(path){for(const name of readdirSync(path)){const full=join(path,name);const info=statSync(full);if(info.isDirectory()){if(name==='node_modules'||name==='.next'||name==='dist')continue;walk(full);continue;}if(!textExtensions.has(extname(name)))continue;checkFile(full);}}
